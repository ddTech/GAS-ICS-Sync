...
* 2) Settings: Change lines 40-64 to be the settings that you want to use
* 3) Install:
...
// 1. Column: ICS url
// 2. Column: display name of the calendar 
// 3. Column: (optional) pre defined color id of the events according to this list:
...
// or:           ["https://www.calendarlabs.com/ical-calendar/ics/76/US_Holidays.ics", "US Holidays", "11"]
...
var howFrequent = 15;                  // What interval (minutes) to run this script on to check for new events.
                                       // Will automatically be adjustet to either 1, 5, 15, 30 minutes or to 
                                       // full hours when >= 45 minutes.
var onlyFutureEvents = false;          // If you turn this to "true", past events will not be synced (this will also removed past events 
                                       // from the target calendar if removeEventsFromCalendar is true)
var addEventsToCalendar = true;        // If you turn this to "false", you can check the log (View > Logs) to make sure your events are being read correctly before turning this on
var modifyExistingEvents = true;       // If you turn this to "false", any event in the feed that was modified after being added to the calendar will not update
var removeEventsFromCalendar = true;   // If you turn this to "true", any event created by the script that is not found in the feed will be removed.
var addAlerts = "yes";                 // Whether to add the ics/ical alerts as notifications on the Google Calendar events 
                                       // or revert to the calendar's default reminders ("yes", "no", "default").
...
var overrideVisibility = "";           // Changes the visibility of the event ("default", "public", "private", "confidential"). 
                                       // Anything else will revert to the class value of the ICAL event.
var addTasks = false;

var emailSummary = false;              // Will email you when an event is added/modified/removed to your calendar
var email = "";                        // OPTIONAL: If "emailSummary" is set to true or you want to receive update notifications, 
                                       // you will need to provide your email address

var gnRespectLastModified = 2;         // Support optimized download of calendars
                                       // 0: no, force download of calendars with each request
                                       // 1: yes, but not when multiple ICS files get merged into one target calendar
                                       // 2: yes, even when multiple ICS files get merged into one target calendar 
                                       //    (might require a second request of one or the other calendar)
...
function install(){
  ...
  // DD 2022-08-28
  // allow for frequencies lower than 30 minutes 
  let nFrequency = getValidTriggerFrequency(howFrequent);
  
  if (nFrequency <= 30){
    ScriptApp.newTrigger("startSync").timeBased().everyMinutes(nFrequency).create();
  } else {
    ScriptApp.newTrigger("startSync").timeBased().everyHour(nFrequency/60).create();
  }
...
function uninstall(){
  ...
  // actually, we could delete all UserProperties
...
function startSync(){
  ...
  // Prepare source URLs to optionally be merged into single target calendars 
  sourceCalendars = condenseCalendarMap(sourceCalendars);

  // DD 2022-08-26
  // Multiple URLs may get merged into a single target calendar.
  // In this case calendar groups are needed for optimized download 
  // of ICS files. -> fetchSourceCalendars()
  let nCalendarGroup = -1;
  ...
  for (var calendar of sourceCalendars){
    ...
    //------------------------ Fetch URL items ------------------------    
    var responses = fetchSourceCalendars(sourceCalendarURLs, nCalendarGroup);

    // DD 2022-08-24
    // Sync only, when new data is available
